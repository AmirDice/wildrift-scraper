...
class FiniteBoard(FakeBoard):
    """A board with only `size` entries -- the NA short-board case (Xin
    Zhao's board ends at 45). No action can scroll below the last row; the
    real device lets the final row settle around y=810 (measured: rank 45
    tapped at y=812 on the live frame), which max_pos = size - 4 reproduces."""

    def __init__(self, size: int, **kw) -> None:
        super().__init__(**kw)
        self.size = size
        self.max_pos = max(1.0, size - 4.0)

    def truth(self) -> dict[int, int]:
        return {r: y for r, y in super().truth().items() if r <= self.size}

    def drag(self, rows: float, pitch: float) -> None:
        self.actions.append(("drag", rows))
        self.pos = min(self.max_pos, max(1.0, self.pos + rows))

    def fling(self, down: bool) -> None:
        self.actions.append(("fling", down))
        step = self.fling_step if down else -self.fling_step
        self.pos = min(self.max_pos, max(1.0, self.pos + step))


def test_short_board_reports_end_instead_of_detection_loss():
    """Hunting rank 46 on a 45-entry board must come back as 'the board ends
    at 45', not as detection loss. The live failure treated it as loss, sent
    unattended recovery against a screen that was never wrong, and the
    recovery's own scrolling then wrecked the carousel's position."""
    board = FiniteBoard(size=45, pos=40.0)
    nav = make_nav(board)
    assert nav.ensure_visible(46) is None
    assert nav.bottom_rank == 45


def test_short_board_last_real_rank_still_resolves():
    """The end-of-board path must not swallow ranks that DO exist: the same
    short board still serves its final row."""
    board = FiniteBoard(size=45, pos=40.0)
    nav = make_nav(board)
    y = nav.ensure_visible(45)
    assert y is not None
    assert abs(y - board.true_y(45)) <= 12
    assert nav.bottom_rank is None
